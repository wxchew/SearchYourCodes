...
try:
    ...
    CONFIG_AVAILABLE = False  # Temporarily disabled to avoid path issues
except ImportError:
...
class CppParser:
    ...
    def _init_queries(self):
        ...
        try:
            # Initialize all queries using the new Query constructor API
            self.function_query = Query(self.language, queries['function'])
            self.class_query = Query(self.language, queries['class'])
            self.namespace_query = Query(self.language, queries['namespace'])
            self.comment_query = Query(self.language, queries['comment'])
        except Exception as e:
            raise RuntimeError(f"Failed to compile Tree-sitter queries: {e}")

    def _execute_query(self, query, node):
        """Execute a tree-sitter query using the new API."""
    ...
    def _extract_file_docstring(self, source_bytes: bytes, tree) -> str:
        ...
        comment_matches = self._execute_query(self.comment_query, tree.root_node)
        comments = []
    ...
    def _add_chunk_if_unique(self, chunks: List[CodeChunk], seen_chunks: set, new_chunk: CodeChunk) -> bool:
        """Add chunk to list only if it's unique. Returns True if added, False if duplicate."""
        # Create a unique signature for the chunk
        chunk_signature = (
            str(new_chunk.file_path),
            new_chunk.start_line,
            new_chunk.end_line,
            new_chunk.function_name,
            new_chunk.class_name
        )
        ...
        if chunk_signature not in seen_chunks:
            ...
            if self.verbose:
                print(f"  Skipping duplicate chunk: {new_chunk.file_path.name}:{new_chunk.start_line}-{new_chunk.end_line}:{new_chunk.function_name or new_chunk.class_name}")
            return False
    ...
    def parse_file(self, file_path: Path) -> List[CodeChunk]:
        ...
        chunks = []
        seen_chunks = set()  # Track processed chunks to avoid duplicates
        ...
        try:
            ...
            class_matches = self._execute_query(self.class_query, tree.root_node)
                            ...
                            if class_name and node.type == 'class_specifier':
                                ...
                                if file_path.suffix.lower() in ['.h', '.hpp', '.hxx']:
                                    ...
                                    self._extract_function_declarations_from_class(
                                        ...
                                        class_namespace, class_docstring or file_summary, chunks, seen_chunks
                                    )
                                ...
                                if file_path.suffix.lower() not in ['.h', '.hpp', '.hxx']:
                                    self._add_chunk_if_unique(chunks, seen_chunks, CodeChunk(
                                        content=content,
                                        file_path=file_path,
            ...
            namespace_dict = {}
            namespace_matches = self._execute_query(self.namespace_query, tree.root_node)
            ...
            function_matches = self._execute_query(self.function_query, tree.root_node)
    ...
    def _extract_function_declarations_from_class(self, class_node, source_bytes: bytes, source_lines: List[str], 
                                                  ...
                                                  class_summary: str, chunks: List[CodeChunk], seen_chunks: set):
    ...
    def parse_repository(self, repo_path: Path, max_files: Optional[int] = None, sample_fraction: float = 1.0) -> List[Dict]:
        """Parse C++ files in repository and return code chunks.
        
        Args:
            repo_path: Path to repository root
            max_files: Maximum number of files to process (None for all)
            sample_fraction: Fraction of files to randomly sample (0.0-1.0)
        """
        chunks = []
        cpp_files = []
        ...
        if sample_fraction < 1.0 or max_files is not None:
            import random
            random.seed(42)  # For reproducible results
            
            if sample_fraction < 1.0:
                sample_size = int(len(cpp_files) * sample_fraction)
                cpp_files = random.sample(cpp_files, min(sample_size, len(cpp_files)))
                print(f"Randomly sampled {len(cpp_files)} files ({sample_fraction:.1%} of total)")
            
            if max_files is not None and len(cpp_files) > max_files:
                cpp_files = random.sample(cpp_files, max_files)
                print(f"Limited to {len(cpp_files)} files (max_files={max_files})")
...
def main() -> int:
    ...
    parser.add_argument(
        '--repo-path', 
        type=Path,
        default=resolve_codebase_path() if CONFIG_AVAILABLE else Path('src/manuel_natcom/src/sim'),
        help='Path to repository root (uses config.py if available)'
    )
    parser.add_argument(
        '--output',
        type=Path,
        default=Path('../data/processed/code_chunks_clean.json'),
        help='Output file path (default: ../data/processed/code_chunks_clean.json)'
    )
    parser.add_argument(
        '--verbose',
        action='store_true',
    ...
    parser.add_argument(
        '--max-files',
        type=int,
        help='Maximum number of files to process (for faster testing)'
    )
    parser.add_argument(
        '--sample-fraction',
        type=float,
        default=1.0,
        help='Fraction of files to randomly sample (default: 1.0 to process all files)'
    )
    ...
    try:
        ...
        chunks = cpp_parser.parse_repository(
            args.repo_path, 
